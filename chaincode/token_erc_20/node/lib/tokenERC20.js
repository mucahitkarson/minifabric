...
class TokenERC20Contract extends Contract {
    ...
    async _transfer(ctx, from, to, value) {
        ...
        console.log(`fromBalanceKey ${fromBalanceKey} fromUpdatedBalance.toString() ${Buffer.from(fromUpdatedBalance.toString())}`);
        console.log(`toBalanceKey ${toBalanceKey} toUpdatedBalance.toString() ${Buffer.from(toUpdatedBalance.toString())}`);
    ...
    async getHistory(ctx, id) {
        ...
        const balanceKey = ctx.stub.createCompositeKey(balancePrefix, [id]);

        let resultsIterator = await ctx.stub.getHistoryForKey(balanceKey);
